...
public class CharacteristicService {
    ...
    /**
     * Calculate life level of the species accrording to his stats
     */
    public int calcLife(@NonNull Species species) {
        log.debug("Calculate life from {}", species);
        int endurance = species.getCharacteristic().getEndurance();
        double coeff = properties.getProtectionCoeff() / 10;
        double life = 100;
        life += life * coeff * endurance;
        life += life * coeff * species.getCharacteristic().getLevel();
        life = BigDecimal.valueOf(life)
                .setScale(0, BigDecimal.ROUND_HALF_UP)
                .doubleValue();
        log.debug("Result life {}", life);
        return (int) life;
    }
    ...
    /**
     * Calculate datave from attacer to defender according to them stats (strength, endurance, level, luck, etc)
     */
    ...
    /**
     * Calculate maximum possible steps count for species
     */
    public int calcSteps(@NotNull Species species) {
        log.debug("Calculate life from {}", species);
        int agility = species.getCharacteristic().getAgility();
        double steps = 5;
        steps += (double) agility / 2;
