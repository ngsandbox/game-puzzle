...
class DataBaseGameApplicationTests {
    ...
    void testCreateSpecies() {
        Species species1 = speciesService.createSpecies(10);
        String species1Id = species1.getId();
        species1 = speciesService.getSpeciesById(species1Id);
        Assertions.assertNotNull(species1, "Species not found by id: " + species1Id);

        Species species2 = speciesService.createSpecies(10);
        species2 = speciesService.getSpeciesByLogin(species2.getLogin());
        Assertions.assertNotNull(species2, "Species not found by login: " + species2.getLogin());

        Species species3 = speciesService.createSpecies(10);
        species3 = speciesService.getSpeciesByLogin(species3.getLogin());
        Assertions.assertNotNull(species3, "Species not found by login: " + species3.getLogin());

        speciesService.addVictim(species1.getLogin(), species2);

        speciesService.addVictim(species3.getLogin(), species1);


        speciesService.removeSpeciesByLogin(species1.getLogin());
        speciesService.removeSpeciesByLogin(species2.getLogin());
        speciesService.removeSpeciesByLogin(species3.getLogin());
        Assertions.assertThrows(NotFoundException.class,
                () -> speciesService.getSpeciesById(species1Id),
                "Species still exist with id: " + species1Id);
    }
